...
static void get_stack_print_output_blazesym(void *ctx, int cpu, void *data, __u32 size)
{
	...
	if (size < sizeof(struct get_stack_trace_t)) {
		...
		struct KSymResolver *resolver = sym_resolver_create();
		if (resolver == NULL) {
			fprintf(stderr, "Fail to reate a resolver\n");
		} else {
			printf("Kern Stack Trace:\n");
			for (i = 0; i < num_stack; i++) {
				const char *name = sym_resolver_find_addr(resolver, e.kern_stack[i]);
				printf("    [%d] %s @ 0x%llX\n", i, name, e.kern_stack[i]);
			}
...
void test_get_stack_raw_tp_blazesym(void)
{
	...
	pb = perf_buffer__new(bpf_map__fd(map), 8, get_stack_print_output_blazesym,
			      NULL, NULL, NULL);
